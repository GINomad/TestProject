...
namespace TestProject.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    {
                        ...
                        var currentUser = UserManager.FindByEmail(model.Email);
                            if(UserManager.IsInRole(currentUser.Id, "MasterAdmin"))
                            {
                                return RedirectToAction("MasterWelcome", "Home");
                            }
                ...
                default:                   
                    ...
                    else
                    {
                        ...
                        if(appUser != null)
                        {
                            ActiveUser.ActiveUsers.Add(new UserLogin() { UserEmail = model.Email, FailedLoginCount = 1 });
                        }
        ...
        {
            var userSession = ActiveUser.UserSessions.FirstOrDefault(x => x.UserId == id);
